...
the user's eating out hard cap from user_config.json.
"""
...
from ynab_assistant import YNABClient, load_user_config, milliunits_to_dollars
...
def get_eating_out_data(year: int = None, month: int = None) -> dict:
    ...
    if not eating_out_config:
        return {"error": "No eating_out spending cap configured in user_config.json"}
